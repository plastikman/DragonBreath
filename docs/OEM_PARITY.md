...
Current as of **v0.6.2** (drying validated on hardware 2026-07-27).
...
| Timed filament drying | **Implemented** | A target plus a bounded 1–12 hour duration with automatic shutoff, driven from the dashboard Dry screen and API v2 (`drying_start` / `drying_stop`). **Material presets are implemented** (PLA 45 °C/6 h, PETG 65 °C/4 h, ABS 70 °C/4 h, Nylon 55 °C/8 h — one tap pre-fills target + duration). Validated on hardware: a cycle enters drying, holds the target, counts the deadline down, and stops cleanly. |
...
| Fan-only filtration mode | **Intentionally omitted** | Stock behavior reverse-engineered: it is not a separate mode but the low end of AUTO mode — the blower runs alone once the printer's bed passes a `filter_temp` (default 30 °C), and the heater only engages at a higher `heater_temp` (default 80 °C). Deliberately not implemented; DragonBreath's auto mode uses a single heat threshold. |
